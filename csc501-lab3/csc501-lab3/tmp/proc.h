...
struct	pentry	{
	...
	/* lock strctures */
	int 	plock;			/*lock on which process is struck*/	    
        int     pinh; 			/*inherited priority value*/
	int 	locks[NLOCKS];		/*locks held by process*/
	int     ltype;			/* lock type accesing the lock*/
	int 	plockret;		/*Return type from the lock*/
	unsigned long 	lq_enter_time; 	/*lock queue enter time*/
