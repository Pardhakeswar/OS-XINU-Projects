...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	/* Initialising data structures for demand paging */

        init_bsm();  /* backing store initialisation*/
        
        init_frm(); /* frames initialisation */
        
        global_page_tables(); /*initialisng the 4 gloabl page tables to amp to 16 MB*/

        create_pagedirectory(NULLPROC);
	
        set_evec(14,(u_long)pfintr); /* page fault interrupt */
	
        write_cr3(proctab[NULLPROC].pdbr);    
	//kprintf(" cr3 values is :0x%08lx\n ",read_cr3());  
       
        enable_paging();
        
        /* End of initilsations */
