...
int	resched()
{
	...
	kprintf("reschedule: value for pdbr :%d for pid is %d \n",nptr->pdbr, currpid);
	write_cr3(nptr->pdbr); /* writing new process pdbr before context switch */
	//kprintf("=======after resched value for pid :%d is %d\n",currpid, nptr->pdbr);
